...
func (s *gnmiServer) Capabilities(
	...
	resp := &gnmiv1.CapabilityResponse{
		...
		SupportedEncodings: []gnmiv1.Encoding{gnmiv1.Encoding_JSON_IETF},
		GNMIVersion:        "0.7.0",
	}
		...
		Update: []*gnmiv1.Update{
			{
				...
				Val: &gnmiv1.TypedValue{
					Value: &gnmiv1.TypedValue_JsonIetfVal{
						JsonIetfVal: []byte(`{"openconfig-system:cpu": {"utilization": 42.5}}`),
					},
				},
			},
		},
	...
	for {
		...
		case <-ticker.C:
			update := &gnmiv1.Update{
				...
				Val: &gnmiv1.TypedValue{
					Value: &gnmiv1.TypedValue_JsonIetfVal{
						JsonIetfVal: []byte(fmt.Sprintf(`{"used": %d}`, time.Now().Second()*100)),
					},
				},
			}
			notification := &gnmiv1.Notification{
