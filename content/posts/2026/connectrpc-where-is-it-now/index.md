...
Worse, it preserved the most frustrating parts of gRPC. Every single request returned a **200 OK**, regardless of whether the server crashed or the resource was missing. It was a baffling design choice that broke the internet's existing contract for observability. You could not rely on standard load balancer metrics, standard browser dev tools, or your generic APM to see if your site was actually healthy. You were forced to use specialized, protocol-aware tooling just to perform basic debugging. If I have to open a dedicated "gRPC-aware" network tab just to see why a login failed, I feel like it hasn't actually earned the "web" part of gRPC-Web naming.
