...
For Go, avoiding the `net/http` package is a rough decision because it makes it harder to use gRPC endpoints alongside other kinds of HTTP APIs and to re-use HTTP middleware. They eventually added a [`ServeHTTP()`](https://pkg.go.dev/google.golang.org/grpc#Server.ServeHTTP) interface to grpc-go as an experimental way to use the HTTP server from the Go standard library but using that method results in [a significant loss of performance](/posts/benchmarking-go-grpc/). Maybe they did it for performance reasons? If so, it's definitely a tradeoff that has split gRPC from the rest of the Go ecosystem.
...
See the full description for optimize_for on the [official protobuf documentation](https://protobuf.dev/programming-guides/proto3/). While these options exist, they aren't actually used for most target languages. In the future I would totally like to see most of `vtprotobuf` be rolled into the standard protobuf compiler for Go and be used if `optimize_for = SPEED`. Integrating `vtprotobuf`-like optimizations into the standard protobuf compiler could offer significant performance gains for Go and there are potentially similar opportunities in other languages as well.
...
Compare it to some of the OpenAPI tooling. This was generated using [Elements](https://github.com/stoplightio/elements), but there are many, many other alternatives that look equally as polished:
...
However, I think the future of gRPC is bright and can be far less ugly. The community is actively addressing these challenges, developing tools like [the buf CLI](https://buf.build/product/cli), [protovalidate](https://github.com/bufbuild/protovalidate) and [protoc-gen-connect-openapi](https://github.com/sudorandom/protoc-gen-connect-openapi) to bridge the gaps and enhance the developer experience. As gRPC matures and [its ecosystem expands](https://trends.google.com/trends/explore?date=all&q=%2Fm%2F04dzxdz,%2Fg%2F11cp5mklv8,RESTful&hl=en), we can anticipate improved tooling, better editor support, and a smoother integration into the frontend world.
